...
if ($display==0)
{
        ...
        foreach my $i (0..$#SQL)
        {
                my $var=$SQL[$i];
                chomp $var;
                my ($JOB,$STATUS,$STARTIME,$ENDTIME,$COMMAND,$PROFILE,$OWNER,$RUN_MACHINE,$EXIT_CODE,$RUNTIME,$HAS_OVERRIDE,$RUN_NUM,$NTRY,$DESCRIPTION,$EVENTSTATUS,$EVENTTXT,$EVENTTEXT,$EVENTSTAMP,$EVENTNUM,$MO           DIF,$INS)=$var =~ /^(.*)§(.*)§(.*)§(.*)§(.*)§(.*)§(.*)§(.*)§(.*)§(.*)§(.*)§(.*)§(.*)§(.*)§(.*)§(.*)§(.*)§(.*)§(.*)§(.*)§(.*)$/;
                        ...
                        foreach my $num (0..$#autorep_j)
                        {
                                ...
                                if ($line_event =~ /^(\s*.*?)\s+([\d\/]{10}\s\d\d\:\d\d\:\d\d|-----)\s+([\d\/]{10}\s\d\d\:\d\d\:\d\d|-----)\s+(\w\w)\s+(.*?)\s+(.*?)*/)
                                {
                                        ...
                                        if  (exists $hash_sql{$job}{$numi})
                                        {
                                                ...
                                                printf("%-68s %-11s %-1s %3s %-19s %-19s %10s %9s %-13s %-9s %+9s\n",$jobname_j,$hash_sql{$job}{$numi}{status},$OVERRIDE,$hash_sql{$job}{$numi}{exit_code},$hash_sq           l{$job}{$numi}{start_time},$hash_sql{$job}{$numi}{end_time},$hash_sql{$job}{$numi}{duration},$ntry,$hash_sql{$job}{$numi}{machine},$hash_sql{$job}{$numi}{modif},$hash_sql{$job}{$numi}{ins});
                                                if ( $info )
                                                {
                                                ...
                                                if ( $event )
                                                {
                                                        ...
                                                        {
                                                                printf ("%-19s %-20s %-15s %-s\n",$hash_sql{$job}{$numi}{$nume}{stamp},$hash_sql{$job}{$numi}{$nume}{txt},$hash_sql{$job}{$numi}{$nume}{status},$ha           sh_sql{$job}{$numi}{$nume}{text},$hash_sql{$job}{$numi}{$nume}{modif},$hash_sql{$job}{$numi}{$nume}{ins});
                                                                $nume++;
                                                        }
                                                }
                                        }
                        ...
                        if ($line_event =~ /^(\s*.*?)\s+([\d\/]{10}\s\d\d\:\d\d\:\d\d|-----)\s+([\d\/]{10}\s\d\d\:\d\d\:\d\d|-----)\s+(\w\w)\s+(.*?)\s+(.*?)*/)
                        {
                                ...
                                if  (exists $hash_sql{$job})
                                {
                                        ...
                                        while (exists $hash_sql{$job}{$cpt})
                                        {
                                                ...
                                                #printf("$TSW_PURPLE%-68s$T_FIN $CSTATUS%-11s$T_FIN %-1s %3s %-19s %-19s %10s %9s %-s\n",$jobname_j,$hash_sql{$job}{$cpt}{status},$OVERRIDE,$hash_sql{$job}{$cpt}{e           xit_code},$hash_sql{$job}{$cpt}{start_time},$hash_sql{$job}{$cpt}{end_time},$hash_sql{$job}{$cpt}{duration},$ntry,$hash_sql{$job}{$cpt}{machine},$hash_sql{$JOB}{$cpt}{modif});
                                                if ( $info )
                                                {
                                                ...
                                                if ( $event )
                                                {
                                                        ...
                                                        {
                                                                printf("%-19s %-20s %-15s %-s\n",$hash_sql{$job}{$cpt}{$nume}{stamp},$hash_sql{$job}{$cpt}{$nume}{txt},$hash_sql{$job}{$cpt}{$nume}{status},$hash_s           ql{$job}{$cpt}{$nume}{text});
                                                                $nume++;
                                                        }
                                                }
                                                $cpt++;
