...
############# DIFF IMAGE PROD MAROC AND COPY MAROC #################

####################################################################

Le present fichier donne les differences entre le serveur s00va9949995 et parva4000634

--------------------------------------------------------------------

"

sdiff -l -w 225 $TRACE $TRACEM
...
TRACEM=${REP}/Photo_parva4000634_${DATE}
OUT=/tmp/image.diff.out
NAME=sdiff_Photo_Serveurs_Prod_Maroc_${DATE}.txt

cd $REP

#Purge des anciens rapports
find $REP -name "Photo_${HOST}*" -type f -mtime +5 -exec rm -f {} \;
find $REP -name "Photo_parva4000634*" -type f -mtime +5 -exec rm -f {} \;
...
#Envoi mail
(echo "Subject: sdiff_Photo_Serveur_${HOST}_${DATE}"; echo; uuencode ${OUT} ${NAME})|sendmail.image -v -f paris_ips_meo_irb -t arnaud.lozahic@externe.bnpparibas.com,fathi.beddiar@bnpparibas.com,AFRIQUE_DSI_RA_VALMEP@bnpparibas.com,paris_ips_meo_irb@bnpparibas.com,francois.burnouf@bnpparibas.com,yuthay.yean@bnpparibas.com,benjamin.kalfon@bnpparibas.com,damien.bursztynowicz@bnpparibas.com
