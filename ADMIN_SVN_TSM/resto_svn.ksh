...
then
	...
	echo "" >> ${out}
	echo "Dans le cas contraire, lancez le job ctlm QDL7_OREST_SVN_TSM_RRI/QDL7ORESTBCK." >> ${out}
	echo "" >> ${out}
	echo "cdt," >> ${out}
	mailx -r mbx-tip-app-bfi-rri@natixis.com -s "[${ENVI}][SVN][${J_DAY}] - Pas de fichier ${rep_backup}/${fic2} a restaurer" ld-tip-app-bgc-rri@natixis.com < ${out}
	exit 1
fi
}
...
then
	...
	echo "Bonjour," > ${out}
        ...
        echo "" >> ${out}
        echo "cdt," >> ${out}
        mailx -r mbx-tip-app-bfi-rri@natixis.com -s "[${ENVI}][SVN][${J_DAY}] - Arret/Relance du service httpd de ${rep1} pour restauration du repository" ld-tip-app-bgc-rri@natixis.com < ${out}
	sleep 120
	Stop -s HTTPD_SVN
else
	...
	echo "" >> ${out}
        echo "cdt," >> ${out}
        mailx -r mbx-tip-app-bfi-rri@natixis.com -s "[${ENVI}][SVN][${J_DAY}] - La restauration de l'archive ${fic} est KO}" ld-tip-app-bgc-rri@natixis.com < ${out}
        exit 1
fi
}
...
then
	...
	echo "La restauration du repertoire ${rep} est OK le $(date +'%d/%m/%Y a %H:%M')" >> ${out}
        ...
        echo "" >> ${out}
        echo "cdt," >> ${out}
        mailx -r mbx-tip-app-bfi-rri@natixis.com -s "[${ENVI}][SVN][${J_DAY}] - Restauration et redemarrage de ${rep1} OK" ld-tip-app-bgc-rri@natixis.com < ${out}
else
