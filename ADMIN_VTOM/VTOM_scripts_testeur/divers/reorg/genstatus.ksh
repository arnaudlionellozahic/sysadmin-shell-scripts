...
echo "- export global : (v)texport [-a] > referentiel_vtom.exp / -a pour prendre en compte les objets non rattaches"
echo "- lancer "$0" (ce dernier va generer le script de mise a jour majstatus.ksh)"
echo "- renommer le repertoire 'bases' actuel"
echo "- creer une base vierge"
echo "- importer le fichier d export global : v4 - timport referentiel_vtom.exp / v5 - vtimport -f referentiel_vtom.exp"
echo "- effectuer le treset des environnements sur l ensemble des dates rattachees (script reseted.ksh genere)"
echo "- lancer le script majstatus.ksh et consultez la log majstatus.log"
...
echo
echo "Environnement $1 inexistant, abandon."
echo
exit 1
}
echo
...
echo
echo "Application $1/$2 inexistante, abandon."
echo
exit 1
}
echo
...
# on positionne filtre_avenir a 1 lorsque'on veut realiser ce controle (non utilise si on ne recupere le statut d'une seule application)

export filtre_avenir=0
...
ls ${TOM_BIN}/vtaddjob 2>/dev/null
if [ $? -eq 0 ] ; then
	v5=1
	bin_job="vtaddjob"
	bin_app="vtaddapp"
...
# --D-- generation du script pour les treset - DEBUT
...
if [ $# -eq 1 ]
then
	tlist env/date | awk ' $1 == ENVIRON["majenv"] {print "echo \"treset env "$1" date "$2"\"";print "treset "$1" "$2}' > reseted.ksh
fi

# --F-- generation du script pour les treset - FIN
...
tlist retenus | awk '{print ENVIRON["bin_job"]" /nom=" $1"/"$2"/"$3" /retenu=oui"}'
fi
...
tlist retenus | awk '$1 == ENVIRON["majenv"] {print ENVIRON["bin_job"]" /nom=" $1"/"$2"/"$3" /retenu=oui"}'
fi
...
tlist retenus | awk '$1 == ENVIRON["majenv"] && $2 == ENVIRON["majapp"] {print ENVIRON["bin_job"]" /nom=" $1"/"$2"/"$3" /retenu=oui"}'
fi
...
echo "echo -----------------------" 
echo "funcmaj 2>&1 | tee majstatus.log" 
echo "echo Fin de la mise a jour" 
echo "echo ---------------------" 
echo "echo" 
echo 'echo "Voulez vous consulter la log majstatus.log ? (Entrer ou Ctrl+c)"' 
echo "read a" 
echo "more  majstatus.log"

}
genere $* >> majstatus_0.ksh
if [ $filtre_avenir -gt 0 ] ; then
	#----- Desactivation des vtaddjob pour les applications a venir"
	fgrep -v -f apps_avenir.txt majstatus_0.ksh > majstatus.ksh
else
	cat majstatus_0.ksh > majstatus.ksh
fi

>majstatus_0.ksh

chmod a+x reseted.ksh 
chmod a+x majstatus.ksh 

echo
echo "Generation terminee"
echo "-------------------"
echo "Une fois effectues le timport et le treset des dates (utilisez reseted.ksh),"
echo "vous pouvez lancer majstatus.ksh et consulter la log majstatus.log"
echo
