...
while read app; do
  grep "FIDELIO_LOT2/$app" $STATS_FILE
	...
	if [ $? -eq 0 ]
	then
		/usr/local/bin/sendmime.sh -s "Statistiques FIDELIO" -t "Bonjour, nous sommes le $DATE_JOUR. Vous trouverez ci joint la liste des statistiques d'exécutions des traitements de la chaîne FIDELIO du $DATE. En cas de probleme, contacter svc-exploit@mondial-assistance.fr. Bonne reception." -f $5 $4 $EXT_FICHIER
