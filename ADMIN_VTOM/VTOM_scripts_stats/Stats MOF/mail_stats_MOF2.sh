...
while read app; do
  grep "MIDDLE_OFFICE/$app" $STATS_FILE
done < $2 > $EXT_FICHIER

	if [ $? -eq 0 ]
	then
		/usr/local/bin/sendmime.sh -s "Statistiques MIDDLE OFFICE" -t "Bonjour, nous sommes le $DATE_JOUR. Vous trouverez ci joint la liste des statistiques d'exécutions des traitements de la chaîne du Middle Office du $DATE. En cas de probleme, contacter svc-exploit@mondial-assistance.fr. Bonne reception." -f $5 $4 $EXT_FICHIER
