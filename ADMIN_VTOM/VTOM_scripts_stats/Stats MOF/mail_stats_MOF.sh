...
DATE=`date +"%d-%m-%Y"`;
EXT_FICHIER="/produit/vtom/v46/stats/stats_MOF_$DATE.csv";
EXT_MAIL_TO="pierre.maurice@mondial-assistance.fr,arnaud.lozahic@mondial-assistance.fr,svc-exploit@mondial-assistance.fr";
...
#	. $ENV_APPLI/param/.env_stats_$1;
	...
	stats_MOF.sh $DATE > /produit/vtom/v46/stats/stats_MOF_$DATE.csv;
	NB_LIGNES=`cat ${EXT_FICHIER} | wc -l | awk '{print( substr ( $1,0,5 ));}'`;
 
	if [ $? -eq 0 ]
	then
		/usr/local/bin/sendmime.sh -s "Statistiques MIDDLE OFFICE" -t "Bonjour, nous somme le $DATE. Vous trouverez ci joint la liste des statistiques d'exécutions des traitements de la chaîne du Middle Office du $DATE. Le fichier contient $NB_LIGNES lignes. En cas de probleme, contacter svc-exploit@mondial-assistance.fr. Bonne reception." -f $EXT_MAIL_FROM $EXT_MAIL_TO $EXT_FICHIER
